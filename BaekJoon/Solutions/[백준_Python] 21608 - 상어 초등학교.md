...
교실의 크기가 N x N이고, N^2의 학생의 번호와 그 학생이 좋아하는 학생의 번호가 담긴 리스트 순열이 주어진다고 할 때, 주어진 조건을 만족하는 학생을 순서대로 앉혀 만족도의 총 합을 구하는 문제입니다.

N의 제한이 3<=N<=20이기 때문에 학생의 번호를 순회(N^2)하면서 해당 학생이 모든 좌석을 앉을 때 조건을 탐색(N^2)한다고 할 때 시간복잡도는 N^4로 20^4라고 할 때 주어진 1초의 시간 제한 내에 가능할 것이라 생각하여 모든 조건을 탐색하는 완전 탐색 알고리즘을 사용하여 구현했습니다.

먼저 입력으로 주어진 데이터를 각각 딕셔너리와 리스트에 저장한 뒤에 입력된 학생 번호 순서대로 순회하면서 인접한 칸의 좋아하는 학생 착석한 수가 현재까지 기록된 좋아하는 학생의 착석 수보다 높으면 해당 좌표로 갱신하고, 만약 같다면 비어있는 칸의 개수가 기록된 비어있는 칸의 개수보다 높을 경우 해당 좌표로 갱신하도록 구현했습니다. 모든 좌석을 순회한 뒤 저장된 좌표에 해당 학생을 착석 시킨 뒤 나머지 순회를 반복합니다.

모든 학생의 순회가 끝난 뒤 다시 한번 모든 좌석을 순회하면서 각 학생에 대한 만족도를 계산하여 최종 만족도를 출력하도록 구현하여 문제 풀이에 성공했습니다.